...
public class GradientPicker : MonoBehaviour
{
    ...
    public static bool Create(
        Gradient original,
        string message,
        GradientEvent onGradientChanged,
    ...
    )
    {
        ...
        if (done)
        {
            ...
            instance.transform.parent.GetChild(0).GetChild(0).GetComponent<TextMeshProUGUI>().text =
                message;
            instance.Setup();
            return true;
        }
    ...
    {
        if (colorKeys.Count < 4) // Changed from 8 to 4
        {
            Slider s = Instantiate(key, transform.position, new Quaternion(), transform)
                .GetComponent<Slider>();
    ...
    {
        /*if (alphaKeys.Count < 4) // Changed from 8 to 4
        {
            Slider s = Instantiate(key, transform.position, new Quaternion(), transform)
                .GetComponent<Slider>();
            ...
            s.gameObject.SetActive(true);
            s.value = k.time;
            s.transform.GetChild(0).GetChild(0).GetChild(0).GetComponent<Image>().color = new Color(
                k.alpha,
                k.alpha,
                k.alpha,
                1f
    ...
    public void SetAlpha(float value)
    {
        if (interact)
        {
            alphaKeys[selectedAlphaKey] = new GradientAlphaKey(
                value,
                alphaKeys[selectedAlphaKey].time
            );
            modifiedGradient.SetKeys(colorKeys.ToArray(), alphaKeys.ToArray());
            CalculateTexture();
            alphaComponent.GetChild(4).GetComponent<InputField>().text = Mathf
                .RoundToInt(value * 255f)
                .ToString();
            alphaKeyObjects[selectedAlphaKey].transform
                .GetChild(0)
                .GetChild(0)
                .GetChild(0)
                .GetComponent<Image>()
    ...
    private void ChangeSelectedColorKey(int value)
    {
        ...
        {
            colorKeyObjects[selectedColorKey].transform
                .GetChild(0)
                .GetChild(0)
                .GetComponent<Image>()
                .color = Color.gray;
        ...
        colorKeyObjects[value].transform.GetChild(0).GetChild(0).GetComponent<Image>().color =
            Color.green;
    ...
    private void ChangeSelectedAlphaKey(int value)
    {
        ...
        alphaKeyObjects[value].transform.GetChild(0).GetChild(0).GetComponent<Image>().color =
            Color.green;
    ...
    public void Select()
    {
        ...
        if (s.name == "ColorKey")
        {
            ...
            positionComponent.text = Mathf
                .RoundToInt(colorKeys[selectedColorKey].time * 100f)
                .ToString();
        ...
        else
        {
            ...
            positionComponent.text = Mathf
                .RoundToInt(alphaKeys[selectedAlphaKey].time * 100f)
                .ToString();
            alphaComponent.GetComponent<Slider>().value = alphaKeys[selectedAlphaKey].alpha;
            alphaComponent.GetChild(4).GetComponent<InputField>().text = Mathf
                .RoundToInt(alphaKeys[selectedAlphaKey].alpha * 255f)
                .ToString();
        }
    }
    ...
    public void SetTime(string time)
    {
        ...
        if (colorComponent.gameObject.activeSelf)
        {
            ...
            colorKeys[selectedColorKey] = new GradientColorKey(
                colorKeys[selectedColorKey].color,
                t
            );
        }
        else
        {
            ...
            alphaKeys[selectedAlphaKey] = new GradientAlphaKey(
                alphaKeys[selectedAlphaKey].alpha,
                t
            );
        }
        modifiedGradient.SetKeys(colorKeys.ToArray(), alphaKeys.ToArray());
    ...
    public void ChooseColor()
    {
        ColorPicker.Create(
            colorKeys[selectedColorKey].color,
            "Gradient Color Key",
            (c) => UpdateColor(selectedColorKey, c),
            null
        );
    }
    ...
    private void UpdateColor(int index, Color c)
    {
        ...
        colorKeyObjects[index].transform
            .GetChild(0)
            .GetChild(0)
            .GetChild(0)
            .GetComponent<Image>()
